...
menu := pm.CreateMenu({"iconssize":16})
...
newmenu1 := pm.CreateMenu({"iconssize":16})
newmenu2 := pm.CreateMenu({"iconssize":16})
newmenu3 := pm.CreateMenu({"iconssize":16})
...
PUM_out( msg, obj )
{
  if ( msg = "onselect" )
  {
    ...
    tooltip,% "Selected: " obj.name "/" obj.id,% rect.right,% rect.top
  }
  if ( msg = "onclose" )
    tooltip
  if ( msg ~= "oninit|onuninit|onshow" )
